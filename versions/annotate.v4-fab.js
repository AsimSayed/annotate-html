...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      // Collapsed FAB (round icon button) — shown when toolbar is hidden
      '#ann-fab{position:fixed;bottom:24px;right:24px;z-index:99999;',
      'width:44px;height:44px;border-radius:50%;',
      'background:#FAF9F5;color:#C96442;',
      'border:1px solid rgba(31,30,29,.08);',
      'box-shadow:0 1px 2px rgba(31,30,29,.04),0 8px 24px rgba(31,30,29,.10);',
      'display:none;align-items:center;justify-content:center;',
      'cursor:pointer;font-family:inherit;padding:0;',
      'transform:scale(.8);opacity:0;',
      'transition:transform .25s cubic-bezier(.2,.8,.2,1),opacity .2s ease,box-shadow .2s ease}',
      '#ann-fab.show{display:flex;transform:scale(1);opacity:1}',
      '@media (hover:hover){#ann-fab:hover{transform:scale(1.06);',
      'box-shadow:0 2px 4px rgba(31,30,29,.06),0 12px 32px rgba(31,30,29,.14)}}',
      '#ann-fab:active{transform:scale(.96)}',
      '#ann-fab svg{width:18px;height:18px;display:block}',
      '#ann-fab .ann-fab-badge{position:absolute;top:-3px;right:-3px;',
      'background:#C96442;color:#FAF9F5;border:1.5px solid #FAF9F5;',
      'border-radius:100px;min-width:18px;height:18px;padding:0 5px;',
      'font-size:10px;font-weight:600;font-variant-numeric:tabular-nums;',
      'display:flex;align-items:center;justify-content:center;line-height:1;',
      'pointer-events:none;transform-origin:center;',
      'transition:transform .2s cubic-bezier(.2,.8,.2,1)}',
      '#ann-fab .ann-fab-badge.empty{display:none}',
      '#ann-fab .ann-fab-badge.bump{animation:ann-badge-pop .35s cubic-bezier(.2,.8,.2,1)}',
      // Crosshair override on FAB in active mode
      'body.ann-active #ann-fab,body.ann-active #ann-fab *{cursor:pointer !important}',
  ...
  function createToolbar() {
    ...
    // Collapsed FAB (round button shown when toolbar is hidden)
    fab = el("button", {
      id: "ann-fab",
      onClick: showToolbar,
    ...
    fab.appendChild(iconEl("pencil"));
    fabBadge = el("span", { className: "ann-fab-badge empty" }, "0");
  ...
  function hideToolbar() {
    ...
    closePopup();
    hideHover();
    // Show FAB after toolbar finishes its fade-out
    if (fab) {
      setTimeout(function () {
  ...
  function showToolbar() {
    ...
    if (fab) {
      ...
      setTimeout(function () { fab.style.display = "none"; }, 220);
    }
  ...
  function updateCount() {
    ...
    if (fabBadge) {
      ...
      if (n === 0) fabBadge.classList.add("empty");
      else fabBadge.classList.remove("empty");
    }
